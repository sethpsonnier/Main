...
# NOTE: Now uses complete custom CSS - NO external CDN dependencies
#========================================================================
...
#------------------------------------------------------------------------
# HTML Chart Generation - Custom CSS Only (No CDN)
#------------------------------------------------------------------------

# Read custom CSS content - REQUIRED for this script to work
$customCssContent = ""
if (Test-Path $customCssPath) {
    try {
        $customCssContent = Get-Content -Path $customCssPath -Raw
        Write-Host "Info: Loaded complete custom CSS from $customCssPath"
    }
    catch {
        Write-Host "ERROR: Could not read custom CSS file: $_"
        Write-Host "ERROR: Script 3 must be run first to create the CSS file!"
        exit 1
    }
} else {
    Write-Host "ERROR: Custom CSS file not found at $customCssPath"
    Write-Host "ERROR: Script 3 must be run first to create the CSS file!"
    exit 1
}
...
<head>
    ...
    <style>
        ...
        /* Complete Custom Chart CSS - No external dependencies */
        $customCssContent
    </style>
</head>
<body>
